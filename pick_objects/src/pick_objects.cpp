...
int main(int argc, char** argv){
  ...
  // select auto or manual model
  char modeValue = '0';
  bool autoMode;

	std::cout<<"|-------------------------------|"<<std::endl;
	std::cout<<"|SLECT A MODE:"<<std::endl;
  ...
  if(autoMode){
    ...
    do{
      choice =choose();
      if (choice == '0'){
        goalReached = moveToGoal(xInitial, yInitial);
      }else if (choice == '1'){
        goalReached = moveToGoal(xStart, yStart);
      }else if (choice == '2'){
        goalReached = moveToGoal(xEnd, yEnd);
...
char choose(){
	...
	std::cout<<"|'0': Initial Position "<<std::endl;
	std::cout<<"|'1': Pick Up Position "<<std::endl;
	std::cout<<"|'2': Drop off Position "<<std::endl;
