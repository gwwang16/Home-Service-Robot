...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr& scan_msg)
{
    ...
    if (nan_count > (range_size * 0.9) || laser_ranges[range_size / 2] < 0.3) {
        crashed = true;
    }
    else {
        crashed = false;
    ...
    ROS_INFO("laser middle: %5.2f \n", laser_ranges[range_size / 2]);
    ROS_INFO("laser range: [%5.2f,%5.2f] \n", range_min, range_max);
    ROS_INFO("left side: %5.2f, right side: %5.2f \n", left_side, right_side);
    ROS_INFO("door state: %d , follow state: %d \n", thats_a_door, following_wall);
}
